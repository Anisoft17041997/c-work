...
void SaisieNote(char *NomFichier)
{
    ...
    while(!feof(FichierAspirant))
    {
        ...
        do{
            ...
            if(InfoAspirant.Note == -1 || InfoAspirant.Note < 0 || InfoAspirant.Note >20)
            {
                printf("\a");
                printf("La note entr%ce est invalide. R%cessayer\n",130,130);
            }
        }while(InfoAspirant.Note == -1 || InfoAspirant.Note < 0 || InfoAspirant.Note >20);
